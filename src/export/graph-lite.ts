...
/**
 * graph-lite.html — the original zero-dependency viewer: embedded data, vanilla canvas force
 * layout, search, kind filter, click-to-highlight neighbors. Kept as the lightweight extra
 * exporter after the Cytoscape redesign took over graph.html ("use this one as well").
 */
export function renderLiteHtml(graph: RevitifyGraph): string {
...
export const graphLiteExporter: Exporter = {
  id: "graph-lite",
  filename: "graph-lite.html",
  render: (graph) => renderLiteHtml(graph),
};
