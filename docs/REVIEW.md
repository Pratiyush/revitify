...
5. ← **NEXT (decisions, not code):** S1 (sync/async contract wording), P2-6 (whole-graph-in-memory is the scale ceiling — name it; streaming output only if someone hits OOM), the incohesive-cluster branch + a large-graph fixture (the `COHESION_MIN=50` path is currently untested).
