...
3. ✅ **DONE.** **D1 — `finalizeGraph` + shared `extractOne` (M):** the verbatim resolve→dedup→summarize→cluster→gitHead tail is now one function in `passes/finalize.ts`; the gated/cached per-file extract is one `extractOne` in `passes/extract.ts` (worker calls it with `cache=undefined`). All four paths (sync, async-sequential, worker, pool) collapse to walk + strategy + shared finalize. Output byte-identical (contract green); worker==sequential still proven; `extract.ts` stays free of the tail so the worker bundle stays lean.
4. ✅ **DONE.** **P1·perf — `GraphIndex.adj` (M):** undirected adjacency is now built once (lazy memo on `GraphIndex`, order-preserving) and shared by all neighbor/path/centrality callers; `neighbors()` is an O(1) lookup, no per-call `Set`. Brandes uses a head-cursor queue (O(1) dequeue, no `shift()`) and a `SKIP_LIMIT=20_000` guard above which betweenness is skipped (questions degrade gracefully). Output byte-identical (report + contract green).
5. ✅ **DONE (decisions, not code):** **S1** — the sync/async language-depth contract is now stated loudly on `buildGraph` and in the architecture guide (sync = TS/JS depth + shallow regex/bare-node fallback elsewhere; async = all languages; same contract shape, may legitimately differ in detail for a non-TS/JS repo). **Scale ceiling** — the whole-graph-in-memory ceiling is named (architecture guide → *Scale ceiling*): betweenness skipped above ~20k nodes, output materialized not streamed, streaming deferred until a real OOM. **Incohesive branch** — exercised by a direct test (`cohesion` + `resplitOversizedAndIncohesive` are exported); documented as a defensive parity port that our deterministic Louvain can't reach on its own output (modularity-negative), so it's driven with a crafted assignment rather than an organic fixture.

**All depth-needed roadmap items (D2, D1, #4, #5) are complete.**
